...
void OrsaModel::bestNFA(const std::vector<ErrorIndex>& e,
                                         double loge0,
                                         double maxThreshold,
                                         const std::vector<float> &logc_n,
                                         const std::vector<float> &logc_k, OrsaModel::ErrorIndex* bestIndex) const
{

  assert(bestIndex);
  const int startIndex = SizeSample();
  const double multError = (DistToPoint()? 1.0: 0.5);
  bestIndex->error = std::numeric_limits<double>::infinity();
  bestIndex->index = startIndex;
  bestIndex->side = 0;
  const size_t n = e.size();
  for(size_t k = startIndex+1; k <= n && e[k-1].error <= maxThreshold && std::isfinite(e[k-1].error); ++k) {
    double logalpha = logalpha0_[e[k-1].side] + multError*log10(e[k-1].error);
    double error = loge0+logalpha*(double)(k-startIndex)+logc_n[k]+logc_k[k];
    if (error < bestIndex->error){
      bestIndex->error = error;
      bestIndex->index  = k;
      bestIndex->side = e[k-1].side;
    }
  }
}
...
double OrsaModel::orsa(std::vector<size_t> & vec_inliers,
  ...
  const double maxThreshold = (desired_precision > 0 ) ?  desired_precision*desired_precision*N2_(0,0)*N2_(0,0): // Square max error
  ...
  for (size_t iter=0; iter < nIter; iter++) {
    ...
    for (size_t k = 0; k < vec_models.size(); ++k) {
      ...
      //ErrorIndex best = bestNFA(vec_residuals, loge0, maxThreshold, vec_logc_n, vec_logc_k);
      bestNFA(vec_residuals, loge0, maxThreshold, vec_logc_n, vec_logc_k, &best);

      if(best.error < 0 && best.error < minNFA) { // A better model was found
        better = true;
        assert(best.index > this->SizeSample());
        assert(best.error < std::numeric_limits<double>::infinity());
        minNFA = best.error;
        side = best.side;
  ...
  if(model_precision)
    *model_precision = denormalizeError(errorMax, side);
